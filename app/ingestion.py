...
def clone_repository(repo_path: str, job_id: str) -> str:
    """Clone an HTTP(S) repository into the workspace assigned to a job."""
    parsed_url = urlparse(repo_path)
    if parsed_url.scheme not in {"http", "https"} or not parsed_url.netloc:
        raise ValueError("repo_path must be a valid HTTP(S) repository URL")
    ...
    try:
        logger.info("Cloning %s into %s", repo_path, workspace_path)
        Repo.clone_from(repo_path, workspace_path)
